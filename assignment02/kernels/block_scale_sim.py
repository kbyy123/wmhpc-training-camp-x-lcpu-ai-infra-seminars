...
def gemm_scale_per_row_col(A: torch.Tensor, B: torch.Tensor,
    ...
    qa, qb = A.double() / sa[:, None], B.double() / sb[:, None]
...
def gemm_scale_along_k(A: torch.Tensor, B: torch.Tensor,
    ...
    """
    M, K = A.shape
    result = torch.zeros((M, B.shape[0]), dtype=torch.float64, device=A.device)
    for block in range(K // SEG):
        sl = slice(block * SEG, (block + 1) * SEG)
        sa, sb = sA[:, block].double(), sB[:, block].double()
        qa, qb = A[:, sl].double() / sa[:, None], B[:, sl].double() / sb[:, None]
        result += (qa @ qb.T) * sa[:, None] * sb[None, :]
    return result
