...
\newenvironment{answer}
    {\par\smallskip\noindent
     \textcolor{accent}{\bfseries 答：}\ignorespaces}
    {\par\medskip}
...
项目 & 你的卡 \\
\midrule
型号 / compute capability & NVIDIA GeForce RTX 5060 Laptop GPU / 12.0 \\
SM 数量 & 26 \\
warp 大小 & 32 \\
shared memory / block & 49152 \\
最大常驻线程 / SM & 1536 \\
显存总量 & 8546484224 \\
max thread / block & 1024
\bottomrule
\end{tabular}
\end{center}
...
\begin{enumerate}[label=(\alph*)]
  ...
  \item 严格串行的迭代算法（每步依赖上一步的结果），即使换一块算力更强的 GPU 也快不了多少。
    \begin{answer}
    正确。GPU 的算力优势主要体现在吞吐量大，而严格串行的算法主要受单步延迟大小约束。
    \end{answer}
...
\begin{answer}
$10^{12}$ FLOP 指的是吞吐而不是延迟，串行算法主要依靠的是延迟。
\end{answer}
...
warp & thread 的调度单位，一次执行 32 个 thread & 计算单元上的一组（32个）lanes & 所有 thread 的寄存器 & `__syncwarp()` \\
block / CTA & 一组可协作的线程 & 一个 SM & shared memory & shared memory、`__syncthreads()` \\
grid & 一个 kernel 运行创建的所有 block 集合 & 整个 GPU 设备 & global memory &  \\
\bottomrule
\end{tabular}
\end{center}
